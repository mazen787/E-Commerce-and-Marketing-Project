...
Sales_Metrics AS (
    ...
    WHERE O.order_status != 'Cancelled' -- ÈäÍÓÈ ÈÓ ÇáÃæÑÏÑÇÊ ÇáÍÞíÞíÉ
...
JOIN Spend_Metrics S ON R.platform = S.platform -- ÈäÑÈØ ÈÇÓã ÇáãäÕÉ
ORDER BY ROAS DESC;
...
WITH Period_Analysis AS (
    ...
    FROM Clean_Fact_Orders o
    JOIN Clean_Dim_Customers c ON o.customer_id = c.customer_id
    WHERE o.order_date BETWEEN '2025-07-15' AND '2025-09-15' -- ãÞÇÑäÉ ÔåÑ ÞÈá æÔåÑ ÈÚÏ
    GROUP BY c.city, 
             CASE 
                WHEN o.order_date < '2025-08-15' THEN '1. Pre-Issue'
                ELSE '2. Post-Issue' 
